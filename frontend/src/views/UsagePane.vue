  ...
  <main class="flex min-w-0 flex-1 flex-col bg-app">
    <header class="flex items-center gap-2 border-b border-edge px-3 py-2">
      ...
      <div>
        ...
        <p class="text-xs text-muted">Calls and estimated spend by model.</p>
      </div>
    </header>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-4">
      <div class="mx-auto max-w-5xl space-y-4">
        <div class="flex flex-wrap items-end gap-3 text-sm">
          <label class="grid gap-1 text-xs text-muted">From
            <input v-model="start" :max="end || undefined" type="date" class="rounded bg-surface2 px-2 py-1.5 text-sm text-base outline-none" />
          </label>
          <label class="grid gap-1 text-xs text-muted">To
            <input v-model="end" :min="start || undefined" type="date" class="rounded bg-surface2 px-2 py-1.5 text-sm text-base outline-none" />
          </label>
          <button v-if="start || end" class="rounded px-2 py-1.5 text-sm text-muted hover:bg-surface2 hover:text-base" @click="clearRange">All time</button>
        </div>

        <p v-if="rangeError" class="text-sm text-red-500" role="alert">Start date must not follow end date.</p>
        <p v-else-if="!rows.length" class="py-12 text-center text-sm text-muted">No usage in this range.</p>
        <div v-else class="overflow-x-auto rounded border border-edge">
          <table class="w-full text-left text-sm">
            <thead class="border-b border-edge bg-surface2 text-xs uppercase tracking-wide text-muted">
              <tr><th scope="col" class="px-3 py-2 font-medium">Model</th><th scope="col" class="px-3 py-2 font-medium">Kind</th><th scope="col" class="px-3 py-2 font-medium">Usage</th></tr>
            </thead>
            <tbody>
