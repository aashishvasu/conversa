...
assert.ok(/current\.input, model: effectiveSettings\(props\.convo\)\.model/.test(researchBlock), 'preparation retry uses the currently selected chat model')
assert.ok(/buildResearchStartBody/.test(researchBlock), 'ResearchBlock uses the research start request helper')
