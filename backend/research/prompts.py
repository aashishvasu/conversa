...
COORDINATOR = """You coordinate an iterative research frontier. Inspect the brief, current tasks, evidence, gaps, and the new worker results. Return JSON only with exactly these keys: action, resolve, prune, add, merge, gaps. action is `continue` or `finish`; resolve, prune, and merge are arrays of task IDs; add is an array of objects with `question` and `reason`; gaps is an array of concise unresolved gaps. Add only tasks that materially improve coverage. Finish when the brief is answerable or budgets/breakers make more gathering unproductive."""
...
PROMPTS = {"search": WORKER, "note": WORKER, "report": REPORT, "verify": VERIFY, "coordinator": COORDINATOR}
